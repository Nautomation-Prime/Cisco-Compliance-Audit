...
def flush_table(table_rows: list[list[str]], out: list[str]) -> None:
    ...
    out.append("<thead><tr>" + "".join(f"<th>{inline_format(cell)}</th>" for cell in header) + "</tr></thead>")
    out.append("<tbody>")
    for row in body:
        out.append("<tr>" + "".join(f"<td>{inline_format(cell)}</td>" for cell in row) + "</tr>")
            ...
            if i + 1 < len(lines) and is_table_delimiter(lines[i + 1]):
                ...
                while i < len(lines):
                    ...
                    if not row_line.startswith("|"):
                        break
                    table_rows.append([cell.strip() for cell in row_line.strip("|").split("|")])
                    i += 1
                flush_table(table_rows, out)
                continue
...
<body>
  <div class=\"banner\">
    <h1 style=\"margin:0;border:0;padding:0;color:white;\">Cisco Compliance Audit Runbook</h1>
    <p>Rendered HTML copy of docs/RUNBOOK.md for operators who do not use Markdown-capable viewers.</p>
  </div>
  <main>
{body}
  </main>
