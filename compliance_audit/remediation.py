    ...
    for line in lines:
        ...
        if not line or line.startswith("!"):
            ...
            if "Remediation script for" in line:
                parts = line.split("(")
                if len(parts) >= 2:
                    hostname_part = parts[0].replace("!", "").replace("Remediation script for", "").strip()
...
def apply_remediation_to_device(
    ...
    try:
        ...
        log.debug(f"Applying {len(commands)} commands to {hostname}")
        ...
        if any(err in output_lower for err in ["invalid", "error", "failed", "incomplete", "% "]):
            # Log the potentially problematic output
            log.warning(f"Possible errors in output from {hostname}:\n{output}")
        ...
        log.debug(f"Save output on {hostname}: {save_output}")

        # Check if save was successful
        if "OK" not in save_output and "built" not in save_output.lower():
            log.warning(f"Unexpected save output on {hostname}: {save_output}")

        result.success = True
        result.duration_secs = round(time.monotonic() - start_time, 1)

    except Exception as exc:
        log.exception(f"Remediation failed on {hostname}")
...
def apply_remediation_scripts(
    ...
    for script_path in script_paths:
        try:
            ...
            log.error(f"Failed to parse {script_path}: {exc}")
        ...
        for script_path, hostname, ip, commands in parsed_scripts:
            ...
            try:
                ...
                if result.success:
                    progress.console.print(
                        f"  [green]✓[/] {hostname} ({ip}) — "
                        f"{result.commands_applied}/{result.total_commands} commands applied "
                ...
                log.exception(f"Failed to apply remediation to {hostname}")
                result = RemediationResult(
                    hostname=hostname,
                    ip=ip,
                    success=False,
                ...
                if connection:
                    try:
                        ...
                        log.debug(f"Disconnected from {hostname}")
                    except Exception:
                        pass
