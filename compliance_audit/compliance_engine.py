...
class AuditResult:
    ...
    @property
    def total(self) -> int:
        """Return the number of non-skipped findings."""
    ...
    def score_pct(self) -> float:
        """Return compliance score percentage across non-skipped findings."""
        t = self.total
        if t == 0:
            return 100.0
...
class ComplianceEngine:
    ...
    def __init__(self, policy: dict):
        """Initialize the engine with the loaded compliance policy tree."""
    ...
    def audit(
        ...
        """Run enabled compliance checks and return an aggregated audit result."""
        result = AuditResult(
    ...
    def _check_services(
        ...
        if _enabled(mp, "service_timestamps_debug"):
            ...
            f.append(
                self._present(
                    cfg, re.escape(exp), "service_timestamps_debug", exp, exp
                )
            )
        ...
        if _enabled(mp, "service_timestamps_log"):
            ...
            f.append(
                self._present(
                    cfg, re.escape(exp), "service_timestamps_log", exp, exp
                )
            )
    ...
    def _check_ip_settings(
        ...
        if _enabled(mp, "ip_http_authentication"):
            method = mp.get("ip_http_authentication", {}).get(
                "method", "local"
            )
            f.append(
                self._present(
    ...
    def _check_ssh(
        ...
        if _enabled(mp, "ssh_version"):
            ...
            # First, try to get SSH version from "show ip ssh" command output
            if data and data.raw_commands.get("show ip ssh"):
                ssh_output = data.raw_commands["show ip ssh"]
                # Look for "SSH Enabled - version X.X" pattern
                for line in ssh_output.splitlines():
                    m = re.search(
                        r"SSH\s+Enabled\s*-\s*version\s+([\d.]+)",
                        line,
                        re.IGNORECASE,
                    )
                    if m:
                        ...
                        # For version 2, accept 2.0 or 1.99 (which is effectively v2)
                        if ver == 2:
                            if (
                                detected_ver.startswith("2.")
                                or detected_ver == "1.99"
                            ):
                                f.append(
                                    Finding(
                                        "ssh_version",
                                        Status.PASS,
                                        f"SSH version {detected_ver} (v2)",
                                    )
                                )
                            else:
                                f.append(
                                    Finding(
                                        "ssh_version",
                                        Status.FAIL,
                                        f"SSH version {detected_ver} (expected v2)",
                                        remediation=f"ip ssh version {ver}",
                                    )
                                )
                        else:
                            # For other versions (e.g., version 1), do exact match
                            if detected_ver.startswith(f"{ver}."):
                                f.append(
                                    Finding(
                                        "ssh_version",
                                        Status.PASS,
                                        f"SSH version {detected_ver}",
                                    )
                                )
                            else:
                                f.append(
                                    Finding(
                                        "ssh_version",
                                        Status.FAIL,
                                        f"SSH version {detected_ver} (expected v{ver})",
                                        remediation=f"ip ssh version {ver}",
                                    )
                                )
                        break

            # Fallback: check running-config for "ip ssh version" line
            if not version_found:
                f.append(
                    self._present(
                        cfg,
                        rf"^ip ssh version\s+{ver}",
                        "ssh_version",
        ...
        if _enabled(mp, "ssh_authentication_retries"):
            max_r = mp.get("ssh_authentication_retries", {}).get(
                "max_retries", 3
            )
    ...
    def _check_aaa(
        ...
        if _enabled(mp, "aaa_authorization_commands"):
            ...
            for level in node.get("levels", [15]):
                ...
                f.append(
                    self._present(
                        cfg, re.escape(expected), name, expected, expected
                    )
                )
                ...
                if all_have_acl:
                    f.append(
                        Finding(
                            ...
                            Status.PASS,
                            "All "
                            f"{len(communities)} SNMP communities have ACL "
                            "restrictions",
                        )
                    )
                else:
        ...
        for banner_type in ("login", "motd", "exec"):
            ...
            if cfg.has_line(rf"^banner {banner_type}\s+"):
                ...
                if req_text:
                    ...
                    if banner_found:
                        ...
                        f.append(
                            Finding(
                                ...
                                Status.FAIL,
                                "banner "
                                f"{banner_type} present but missing required "
                                f"text: '{req_text}'",
                                remediation=(
                                    f"banner {banner_type} - "
                                    f"must include '{req_text}'"
                                ),
                            )
                        )
                else:
                    f.append(
                        Finding(
                            key, Status.PASS, f"banner {banner_type} present"
                        )
                    )
            else:
                f.append(
    ...
    def _check_vty_lines(
        ...
        if not vty_sections:
            f.append(
                Finding("vty_lines", Status.WARN, "No VTY line config found")
            )
            return f
        ...
        for sec_name, lines in vty_sections.items():
            ...
            if _enabled(mp, "vty_transport_input_ssh"):
                has_ssh_only = any(
                    re.match(r"transport input ssh", line, re.I)
                    for line in lines
                )
                if has_ssh_only:
                    f.append(
            ...
            if _enabled(mp, "vty_exec_timeout"):
                ...
                if timeout_lines:
                    m = re.match(
                        r"exec-timeout\s+(\d+)\s+(\d+)", timeout_lines[0]
                    )
                    if m:
            ...
            if _enabled(mp, "vty_access_class"):
                ...
                if has_acl:
                    ...
                    remed = (
                        f"access-class {acl} in"
                        if acl
                        else "access-class <acl> in"
                    )
                    f.append(
                        Finding(
                            "vty_access_class",
            ...
            if _enabled(mp, "vty_login_authentication"):
                ...
                has_auth = any(
                    re.match(r"login authentication", line, re.I)
                    for line in lines
                )
                if has_auth:
                    f.append(
                        ...
                        if prio <= expected:
                            ...
                            f.append(
                                Finding(
                                    ...
                                    Status.FAIL,
                                    "VLAN "
                                    f"{vid} bridge priority {prio} > expected "
                                    f"{expected} ({label})",
                                    remediation=(
                                        "spanning-tree vlan "
                                        f"{vid} priority {expected}"
                                    ),
                                )
                            )
            if checked_vlans:
                    ...
                    if actual <= expected:
                        f.append(
                            Finding(
                                ...
                                Status.PASS,
                                "VLAN "
                                f"{vlan_spec} STP priority {actual} <= "
                                f"{expected} ({label})",
                            )
                        )
                    else:
    ...
    def _check_vtp(
        ...
        if data.vtp:
            ...
            if actual == expected.lower():
                f.append(
                    Finding("vtp_mode", Status.PASS, f"VTP mode: {actual}")
                )
            else:
                f.append(
                    Finding(
    ...
    def _check_interfaces(
        ...
        f: list[Finding] = []
        # Skip all data plane checks if data_plane is not in policy
        # (for example when category filtering is active).
    ...
    def _check_access_port(
        self, cfg: ParsedConfig, dp: dict, pi: PortInfo
    ) -> list[Finding]:
        f: list[Finding] = []
        intf = pi.name
        ...
        if _enabled(dp, "access_vlan_set"):
            bad_vlans = dp.get("access_vlan_set", {}).get(
                "disallowed_vlans", [1]
            )
        ...
        if _enabled(dp, "port_security"):
            ...
            if pi_has(pi, r"switchport port-security"):
                ...
                if max_mac:
                    mac_line = [
                        cfg_line
                        for cfg_line in pi.config_lines
                        if re.search(
                            r"port-security maximum", cfg_line, re.I
                        )
                    ]
                    if mac_line:
                ...
                if violation:
                    if pi_has(
                        pi,
                        rf"port-security violation\s+{re.escape(violation)}",
                    ):
                        f.append(
                            Finding(
    ...
    def _check_endpoint_trunk_port(
        ...
        if _enabled(dp, "trunk_allowed_vlans"):
            if (
                pi.trunk_allowed_vlans
                and pi.trunk_allowed_vlans.lower() != "all"
            ):
                f.append(
                    Finding(
                        "trunk_allowed_vlans",
        ...
        if _enabled(dp, "trunk_native_vlan"):
            ...
            if trunk_vlan_node.get("on_endpoint_trunks", True):
                # Use endpoint-specific expected VLAN when configured;
                # otherwise fall back to the default value.
                expected_native = trunk_vlan_node.get(
                    "expected_vlan_endpoint",
                ...
                if data and data.switchports:
                    sw_data = data.switchports.get(
                        intf
                    ) or data.switchports.get(pi.name)
                    ...
                    if native_vlan == expected_native:
                        ...
                        f.append(
                            Finding(
                                "trunk_native_vlan",
                                Status.FAIL,
                                f"{intf}: native VLAN {native_vlan}, "
                                f"expected {expected_native} ({neighbor_desc})",
                                interface=intf,
                                remediation=(
    ...
    def _check_trunk_port(
                ...
                if pi_has(pi, r"spanning-tree guard root"):
                    f.append(
                        Finding(
                            "root_guard",
                            Status.WARN,
                            f"{intf}: root guard present but direction "
                            "unknown - verify manually",
                            interface=intf,
                        )
                    )
                # If root guard is not present and direction is unknown,
        ...
        if _enabled(dp, "dhcp_snooping_trust"):
            ...
            want_trust = (
                is_uplink and trust_node.get("on_uplinks", True)
            ) or (is_downlink and trust_node.get("on_downlinks", True))
        ...
        if _enabled(dp, "trunk_native_vlan"):
            ...
            want_check = (
                is_uplink and trunk_vlan_node.get("on_uplinks", True)
            ) or (is_downlink and trunk_vlan_node.get("on_downlinks", True))
            if want_check:
                    ...
                    if native_vlan == expected_native:
                        ...
                        f.append(
                            Finding(
                                ...
                                Status.FAIL,
                                f"{intf}: native VLAN {native_vlan}, "
                                f"expected {expected_native} ({direction})",
                                interface=intf,
                                remediation=(
                                    "switchport trunk native vlan "
                                    f"{expected_native}"
    ...
    def _check_unused_port(
        ...
        parking = self.policy.get("_audit_settings", {}).get(
            "parking_vlan", 999
        )
            ...
            if pi.access_vlan != parking:
                f.append(
                    Finding(
                        "unused_parking_vlan",
                        Status.FAIL,
                        f"{intf}: unused port not in parking VLAN "
                        f"{parking} (is {pi.access_vlan})",
                        interface=intf,
    ...
    def _check_storm_control(self, dp: dict, pi: PortInfo) -> list[Finding]:
        """Check storm-control levels against speed-based thresholds.

        Validates both rising and falling thresholds when configured.
        """
        f: list[Finding] = []
        ...
        # Check whether threshold validation is enabled.
        ...
        if tier is None:
            ...
            for spd in sorted(
                thresholds.keys(), key=lambda x: int(x), reverse=True
            ):
                if speed >= int(spd):
                    tier = thresholds[spd]
                    break
        ...
        if check_thresholds:
            for sc_type in node.get("types", ["broadcast", "multicast"]):
                ...
                pattern = (
                    rf"storm-control\s+{re.escape(sc_type)}\s+level\s+"
                    r"([\d.]+)(?:\s+([\d.]+))?"
                )
                match = None
                for line in pi.config_lines:
                ...
                if match:
                    ...
                    falling_ok = True
                    if (
                        expected_falling is not None
                        and actual_falling is not None
                    ):
                        falling_ok = actual_falling <= expected_falling
                    elif (
                        expected_falling is not None and actual_falling is None
                    ):
                    ...
                    if rising_ok and falling_ok:
                        if (
                            actual_falling is not None
                            and expected_falling is not None
                        ):
                            f.append(
                                Finding(
                                    "storm_control",
                                    Status.PASS,
                                    f"{intf}: {sc_type} storm-control "
                                    f"{actual_rising}%/{actual_falling}% "
                                    f"<= {expected_rising}%/{expected_falling}% "
                                    f"({speed}Mbps tier)",
                                    interface=intf,
                                )
                            )
                            ...
                            f.append(
                                Finding(
                                    ...
                                    Status.PASS,
                                    f"{intf}: {sc_type} storm-control "
                                    f"{actual_rising}% <= {expected_rising}% "
                                    f"({speed}Mbps tier)",
                                    interface=intf,
                                )
                            )
                        ...
                        if not rising_ok and not falling_ok:
                            fail_msg = (
                                f"{intf}: {sc_type} storm-control "
                                f"{actual_rising}%/{actual_falling or 'N/A'}% "
                                f"> {expected_rising}%/{expected_falling}% "
                                f"({speed}Mbps tier)"
                            )
                        elif not rising_ok:
                            fail_msg = (
                                f"{intf}: {sc_type} storm-control rising "
                                f"{actual_rising}% "
                            ...
                            if actual_falling is None:
                                fail_msg = (
                                    f"{intf}: {sc_type} storm-control falling "
                        ...
                        remediation_cmd = (
                            f"storm-control {sc_type} level {expected_rising}"
                        )
                    ...
                    f.append(
                        Finding(
                            "storm_control",
                            Status.FAIL,
                            f"{intf}: {sc_type} storm-control not configured "
                            f"({speed}Mbps tier)",
                            interface=intf,
                            remediation=remediation_cmd,
                        )
        ...
        # Storm-control action is optional.
        # Defaults to true for backward compatibility.
        ...
        if check_action:
            ...
            if expected_action:
                if pi_has(
                    pi, rf"storm-control action\s+{re.escape(expected_action)}"
                ):
                    f.append(
                        Finding(
            ...
            for vid in non_root_vlans:
                f.append(
                    Finding(
                        "core_stp_root",
                        Status.WARN,
                        "Core switch is NOT the STP root bridge for VLAN "
                        f"{vid} - verify",
                        "role_specific",
                    )
                )
        ...
        if _enabled(asw_pol, "stp_not_root") and data.stp:
            ...
            for mode_data in data.stp.values():
                ...
                for vid, vinfo in mode_data.get("vlans", {}).items():
                    ...
                    if (
                        root.get("address")
                        and bridge.get("address")
                        and root["address"] == bridge["address"]
                    ):
                        root_vlans.append(vid)
                ...
                for vid in root_vlans:
                    f.append(
                        Finding(
                            "asw_not_root",
                            Status.FAIL,
                            "Access switch IS the STP root for VLAN "
                            f"{vid} - this should be the core!",
                            "role_specific",
                        )
                    )
            else:
