...
from .auditor import run_audit, load_compliance_config
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        action="store_true",
        help="Approve all pending remediation review packs (requires --approver and --ticket-id).",
    )
    p.add_argument(
        "--remediation-reject",
        metavar="PACK_ID",
    ...
    p.add_argument(
        "--apply-dry-run",
        action="store_true",
        help="Run preflight for remediation apply operations without sending configuration.",
    )
    p.add_argument(
        "--allow-high-risk",
        action="store_true",
...
def _handle_remediation_mode(args: argparse.Namespace) -> bool:
    ...
    if args.remediation_list is not None:
        ...
        print()
        print("=" * 120)
        print(f"{'PACK ID':36} | {'STATUS':8} | {'HOSTNAME':24} | {'IP':15} | {'RISK':11} | {'FINDINGS':8} | {'CREATED'}")
        print("=" * 120)

        for row in rows:
            print(
                f"{row.pack_id} | {row.status:8} | {row.hostname:24} | "
                f"{row.ip:15} | risk={row.highest_risk:6} | findings={row.findings_count:3} | "
    ...
    if args.remediation_approve_all:
        ...
        print(f"Found {len(rows)} pending remediation pack(s):")
        for row in rows:
            print(
                f"  {row.pack_id} | {row.hostname:24} | {row.ip:15} | "
                f"risk={row.highest_risk:6} | findings={row.findings_count:3}"
            )
        ...
        print(f"\nBulk approval complete: {approved_count} approved, {failed_count} failed")
        return True
    ...
    if args.remediation_apply:
        ...
        summary = apply_approved_pack(
            ...
            allow_high_risk=(args.allow_high_risk or not rem.get("execution_block_high_risk", True)),
